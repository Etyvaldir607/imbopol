...
$cliente = $db->select('nombre_cliente, nit_ci, count(nombre_cliente) as nro_visitas, sum(monto_total) as total_ventas')->from('inv_egresos')->where(array('nombre_cliente' => $nombre,'nit_ci' => $nit))->fetch_first();

$movimientos = $db->select('c.codigo, c.nombre, c.color, c.descripcion, d.categoria, SUM(b.cantidad) AS cant, COUNT(a.id_egreso) AS reg, SUM(b.precio) AS prec')->from('inv_egresos a')->join('inv_egresos_detalles b','a.id_egreso = b.egreso_id')->join('inv_productos c','b.producto_id = c.id_producto')->join('inv_categorias d', 'c.categoria_id = d.id_categoria')->group_by('id_producto')->where(array('a.nombre_cliente' => $nombre, 'nit_ci' => $nit))->fetch();
		...
		<div class="col-sm-6">
			<div class="well">
				...
				<dl class="margin-none">
					...
					<dt>total:</dt>
					<dd><?= escape($cliente['total_ventas']); ?></dd>
					<dt></dt>
					<dd></dd>
				</dl>
			</div>
		...
		<table class="table table-bordered table-condensed table-striped table-hover">
			<thead>
				<tr class="active">
					...
					<th class="text-nowrap text-center text-middle" colspan="3">Detalle</th>
					<th class="text-nowrap text-center text-middle" colspan="3">Saldos</th>
				</tr>
				<tr class="active">
					<th class="text-nowrap text-center text-middle">Color</th>
					<th class="text-nowrap text-center text-middle">Tipo</th>
					<th class="text-nowrap text-center text-middle">descripción</th>
			...
			<tbody>
				...
				<?php foreach ($movimientos as $nro => $movimiento) { ?>
					<tr>
						...
						<td class="text-nowrap text-right info text-primary"><strong><?= escape($movimiento['color']); ?></strong></td>
						...
						<td class="text-nowrap text-right success"><?= escape($movimiento['cant']) ?></td>
						<td class="text-nowrap text-right success"><?= escape($movimiento['prec']) ?></td>
					</tr>
				<?php } ?>
			</tbody>
		</table>
