...
if ($id_egreso == 0) {
	...
	// Obtiene los egreso
	$factura = $db->select('i.*, a.almacen, a.principal, e.nombres, e.paterno, e.materno')
				  ->from('inv_egresos i')
				  ->join('inv_almacenes a', 'i.almacen_id = a.id_almacen', 'left')
				  ->join('sys_empleados e', 'i.empleado_id = e.id_empleado', 'left')
...
if ($id_egreso == 0) {
	...
	$pdf->Cell(0, 10, 'LISTA DE FACTURAS', 0, true, 'C', false, '', 0, false, 'T', 'M');
	...
	foreach ($facturas as $nro => $factura) {
		...
		$body .= '<td>' . escape($factura['nombre_cliente']) . '</td>';
		$body .= '<td>' . escape($factura['nit_ci']) . '</td>';
		$body .= '<td align="right">' . escape($factura['nro_factura']) . '</td>';
		$body .= '<td align="right">' . escape($factura['monto_total']) . '</td>';
		$body .= '<td align="right">' . escape($factura['nro_registros']) . '</td>';
		$body .= '<td>' . escape($factura['almacen']) . '</td>';
		$body .= '<td>' . escape($factura['nombres'] . ' ' . $factura['paterno'] . ' ' . $factura['materno']) . '</td>';
		$body .= '</tr>';
	}
...
EOD;
	...
	// Titulo del documento
	$pdf->Cell(0, 10, 'FACTURA # ' . escape($factura['nro_factura']), 0, true, 'C', false, '', 0, false, 'T', 'M');
	...
	$valor_fecha = escape(date_decode($factura['fecha_proforma'], $_institution['formato']) . ' ' . $factura['hora_proforma']);
	$valor_nombre_cliente = escape($factura['nombre_cliente']);
	$valor_nit_ci = escape($factura['nit_ci']);
	$valor_nro_proforma = escape($factura['nro_factura']);
	$valor_monto_total = escape($factura['monto_total']);
	$valor_nro_registros = escape($factura['nro_registros']);
	$valor_almacen = escape($factura['almacen']);
	$valor_empleado = escape($factura['nombres'] . ' ' . $factura['paterno'] . ' ' . $factura['materno']);
	$valor_moneda = $moneda;
	$total = 0;
...
EOD;
	...
	// Genera el nombre del archivo
	$nombre = 'proforma_' . $id_egreso . '_' . date('Y-m-d_H-i-s') . '.pdf';
}
